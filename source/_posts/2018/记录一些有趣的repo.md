---
title: 记录一些有趣的repo
date: 2018-05-31 20:22:58
tags: '优秀项目'
categories: '杂项'
copyright: true
---
[大量开源项目hellogithub](https://hellogithub.com/)

#	轮子
[awesome-javascript](https://github.com/sorrycc/awesome-javascript#package-managers)
[前端常用插件、工具类库汇总](https://juejin.im/post/5ba7d5dd5188255c6140cc9d)
[Web开发前端模版](https://github.com/h5bp/html5-boilerplate)
[美化代码-prettier](https://github.com/prettier/prettier)
[Web技术的终端](https://github.com/zeit/hyper)
[交互式地图](https://github.com/Leaflet/Leaflet)
[剪贴板](https://github.com/zenorocha/clipboard.js/)
[html的head标签](https://github.com/headjs/headjs)
[CSS-Hover.css](https://github.com/IanLunn/Hover)
[视差滚动库](https://github.com/Prinzhorn/skrollr)
[轻量级富文本编辑器](https://github.com/quilljs/quill)
[markDown生成流程图](https://github.com/knsv/mermaid)
[marked](https://github.com/markedjs/marked)
[键盘事件库-hotkeys](https://github.com/jaywcjlove/hotkeys)
[实现数字增长动画-countUp.js](https://github.com/inorganik/CountUp.js)
[Web水印](https://github.com/loadchange/gwm)
[搭建标准的js库jslib-base](https://github.com/yanhaijing/jslib-base)
[draggable](https://github.com/Shopify/draggable)
[表格](https://github.com/SheetJS/js-xlsx)
[发布订阅模式-EventEmitter](https://github.com/Olical/EventEmitter)
[同构fetch](https://github.com/matthew-andrews/isomorphic-fetch)
[不蒜子](http://busuanzi.ibruce.info/)
#	微信小程序开发框架
[vue风格mpvue](https://github.com/Meituan-Dianping/mpvue)
[vue风格wepy](https://github.com/Tencent/wepy)
[react风格taro](https://github.com/NervJS/taro)
#	React相关
[react-hover](https://www.npmjs.com/package/react-hover)
[autobind-decorator](https://www.npmjs.com/package/autobind-decorator)	
[react-helmet](https://www.npmjs.com/package/react-helmet)
[react-loadable](https://www.npmjs.com/package/react-loadable)
[react-content-loader](https://www.npmjs.com/package/react-content-loader)
[react-lazyload](https://www.npmjs.com/package/react-lazyload)
[react-slick](https://react-slick.neostack.com/docs/get-started)
[awesome-react](https://github.com/enaqx/awesome-react)
[react-component-list](https://dvemac.github.io/react-component-list/)
[React-Clipboard](https://www.npmjs.com/package/react-clipboard.js)
[react-dnd-html5-backend](https://www.npmjs.com/package/react-dnd-html5-backend)
[react-dnd](https://www.npmjs.com/package/react-dnd)
[react-highlight-words](https://github.com/bvaughn/react-highlight-words)
#	图标
[Font-Awesome](https://github.com/FortAwesome/Font-Awesome)

#	页面生产图片
[html2canvas](https://github.com/niklasvh/html2canvas/)   
[dom-to-image](https://github.com/tsayen/dom-to-image)  
[rasterizeHTML.js](https://github.com/cburgmer/rasterizeHTML.js)  
#	动画
[Animate.css](https://github.com/daneden/animate.css)
[演示文稿-reveal.js](https://github.com/hakimel/reveal.js)
[演示文稿-impress.js](https://github.com/impress/impress.js)
[animate-text](https://github.com/qgh810/animate-text)
[pdf.js](https://github.com/mozilla/pdf.js)
[轮播器-swiper](https://github.com/nolimits4web/swiper)
#	图表库
[ECharts](https://github.com/apache/incubator-echarts)
[BizCharts](https://github.com/alibaba/BizCharts/)
[AntV](http://antv.alipay.com/zh-cn/index.html)
[Chart](https://github.com/chartjs/Chart.js)
#	游戏
[phaser](https://github.com/photonstorm/phaser)

#	数学库
[mathjs](https://github.com/josdejong/mathjs)
[big.js](https://github.com/MikeMcl/big.js)
[TheoremJS](https://github.com/arguiot/TheoremJS)
[数学公式库-KaTeX](https://github.com/Khan/KaTeX)
#	best-resume-ever
用Vue和LESS简单、快速建立许多漂亮的简历，并创建你最好的简历。
[best-resume-ever](https://github.com/salomonelli/best-resume-ever)
#	css知识	
[You-need-to-know-css](https://github.com/l-hammer/You-need-to-know-css)
#	Web性能测试工具
[hiper](https://github.com/pod4g/hiper)
#	Web页面加载进度条
[nprogress](https://github.com/rstacruz/nprogress/)
[nanobar](https://github.com/jacoborus/nanobar)
#	UI库
[阅文-lulu（jQuery）](https://github.com/yued-fe/lulu)
#	常用的网址
[码点查询大全](https://codepoints.net/)	
#	CSS实现各种形状
[CSS实现各种形状](https://css-tricks.com/examples/ShapesOfCSS/)
#	屏幕录制
[屏幕录制-LICEcap](https://licecap.en.softonic.com/)
[mac-屏幕录制giphy capture](https://giphy.com/apps/giphycapture)

#	代码gitignore
[gitignore](https://github.com/github/gitignore)
#	时间处理
[moment](https://github.com/moment/moment/)
[date-fns](https://github.com/date-fns/date-fns)
[dayjs](https://github.com/iamkun/dayjs)
#	抓取工具
Wireshark、Fiddler、Charles、Postman

